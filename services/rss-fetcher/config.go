		...
		Feeds: []FeedConfig{
			{
				...
				OutletName: "BBC World",
				Interval:   15,
				Category:   "world",
			},
			{
				URL:        "https://rss.cnn.com/rss/cnn_topstories.rss",
				OutletName: "CNN Top Stories",
				Interval:   15,
				Category:   "general",
			},
			{
				URL:        "https://feeds.reuters.com/reuters/topNews",
				OutletName: "Reuters Top News",
				Interval:   10,
				Category:   "general",
			},
		},
...
func (f *RSSFetcher) SyncFeedsWithDatabase(config *FeedsConfig) error {
	...
	for _, feed := range config.Feeds {
		query := `
			INSERT INTO rss_feeds (url, outlet_name, fetch_interval_minutes, active)
			VALUES ($1, $2, $3, TRUE)
		...
		_, err := f.db.Exec(query, feed.URL, feed.OutletName, feed.Interval)
		if err != nil {
