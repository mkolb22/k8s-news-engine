-- Database Column Standardization Migration
-- Standardizes all outlet-related columns to 'outlet_name' with VARCHAR(255)
-- 
-- Before Migration State:
-- - articles.outlet (TEXT)
-- - rss_feeds.outlet (TEXT) 
-- - outlet_reputation_scores.outlet (VARCHAR)
-- - news_agency_reputation_metrics.outlet_name (VARCHAR) ✓
-- - outlet_authority.outlet_name (VARCHAR) ✓
--
-- After Migration State:
-- - All tables use 'outlet_name' VARCHAR(255) NOT NULL
--
-- Migration Strategy: Add new columns, migrate data, maintain backward compatibility
...
-- Rename outlet to outlet_name in outlet_reputation_scores (if needed)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        -- Copy data from outlet to outlet_name
        UPDATE outlet_reputation_scores SET outlet_name = outlet WHERE outlet_name IS NULL;
...
\echo '✅ Database column standardization migration completed successfully!'
\echo '📋 Summary: Standardized outlet columns to outlet_name VARCHAR(255) across all tables'
\echo '🔄 Next Step: Update application code to use outlet_name instead of outlet'
\echo '⚠️  Note: Old outlet columns maintained for backward compatibility during transition'